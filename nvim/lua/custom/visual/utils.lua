...
---Export pokemon colors to env file for use by colorscheme, tmux, and shell
---This file is read by colorscheme.lua to apply pokemon colors to UI elements
...
function M.export_pokemon_to_env(name, shiny, form, colors, background_mode)
  ...
  -- TokyoNight Night palette defaults (matching colorscheme.lua)
  ...
  local env_content = string.format(
    [[# Auto-generated by Neovim pokemon color system
# Pokemon colors for colorscheme, tmux, and shell integration
# Regenerated when pokemon colors are generated or changed
...
export POKEMON_COLOR_BRIGHT="%s"
  ...
  -- Write to cache file (this file is read by colorscheme.lua on_highlights)
  ...
  vim.fn.jobstart(cmd, {
    on_exit = function(_, exit_code, _)
      ...
      if new_colors then
        ...
        -- 1. Triggers the ColorScheme autocommand event
        -- 2. Properly invalidates highlight caches
        -- 3. Ensures LazyVim integration works correctly
        -- 4. Re-runs the on_highlights function to read fresh pokemon-colors.env
        vim.schedule(function()
          -- Reload the colorscheme (no need to manually unload modules)
          vim.cmd("colorscheme tokyonight")

          vim.notify("Colorscheme reloaded with new pokemon colors", vim.log.levels.INFO)
        end)
