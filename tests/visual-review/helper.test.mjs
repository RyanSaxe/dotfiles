...
const skill = path.resolve(
  ...
  "../../ai-harness/skills/visual-review",
);
...
async function open(t) {
  ...
  assert.equal(
    ...
    "a new session is named after its directory",
  );
  const { token } = JSON.parse(
    await fs.readFile(sessionDir + "/connection.json", "utf8"),
  );
...
test("the helper runs when invoked through a symlinked skill directory", async (t) => {
  const directory = await fs.mkdtemp(
    path.join(os.tmpdir(), "visual-review-link-"),
  );
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
  const link = path.join(directory, "skill");
  await fs.symlink(skill, link);
  // A guard that compares path spellings exits 0 here having run nothing;
  // the usage error proves main() ran.
  await assert.rejects(
    run(process.execPath, [path.join(link, "scripts/session.mjs"), "state"]),
