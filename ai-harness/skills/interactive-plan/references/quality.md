...
A page starts at its decision or its material, with no lede and no rationale
before it. Each option has one line of consequence. A sentence that could
...
reviewer settles them in one round. A revision with one question on one page
has not explored enough to ask the rest.

Assemble the final plan from Agreed: a decision that is not on Agreed is not
settled and cannot bind a step. Each revision closes open points. When the
user asks you to decide, decide, record it on Agreed, and go on. When the
user asks for a certainty the project cannot give before implementation, say
what implementation will find out, how, and what result is acceptable. Two
revisions in a row that close nothing mean the plan is ready to present with
its open items named.
...
Silence on a passage is agreement, so the diff is what the user reads next.
Remove a decision or question in the revision that records its answer on
Agreed. A control that stays looks unanswered and gets answered again. If
