...
- **GitHub submission sends comments only.** Notes and local replies are never submitted. Only comment thread bodies and optional suggestions are submitted.
...
4. **Generate the review.** Walk relevant files for the scope. Produce threads per [`references/schema.md`](references/schema.md). Each thread gets a stable `rev-NNN` id, `type`, `author: ai`, severity, confidence, category, body, optional suggestion for comments only, `status: open`, `anchor_text`, `anchor_status: current`, and `replies: []`.
