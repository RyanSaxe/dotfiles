...
export function createRepository({ root, git, cacheDir }) {
  ...
  async function served(cacheDirectory) {
    ...
    for (const name of await fs.readdir(cacheDirectory).catch(() => [])) {
      ...
      // The path index lists every tracked file at a ref; the export shows
      // only the files the session served, so the index stays behind.
      if (entry && !entry.key.startsWith("index:"))
...
export async function serve(
  ...
  try {
    ...
    let state = (await exists(stateFile))
      ...
      : {
          // The directory's name, so the two never differ; a name the id
          // pattern rejects gets a random id instead.
          sessionId: idPattern.test(path.basename(directory))
    ...
    // When the agent last polled or acted, so the browser can say when no
    // agent is listening rather than spin.
    ...
    server = http.createServer(async (req, res) => {
      ...
      try {
        ...
        if (req.method === "GET" && url.pathname === "/api/state")
          return reply(200, { ...state, agentSeenAt });
        ...
        if (req.method === "POST") {
          ...
          if (agent) agentSeenAt = timestamp();
          requireValue(
