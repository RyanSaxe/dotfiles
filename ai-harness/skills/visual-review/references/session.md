...
If a session already exists, resume it with `start --session-dir PATH` rather
than creating a replacement. A live owner prevents a second writer. After an
abnormal shutdown, inspect the recorded owner before using `--recover-lock`;
do not delete ownership files blindly.
