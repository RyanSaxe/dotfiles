...
async function load() {
  const [markdownIt, container, purify] = await Promise.all([
    module_(libraries.markdownIt),
    module_(libraries.container),
    module_(libraries.purify),
  ]);
  return {
...
export async function createRenderer() {
  ...
  md.renderer.rules.fence = (tokens, index) => {
    ...
    if (block.name === "notes") {
      ...
      const where = target.ref ? `${target.path}@${target.ref}` : target.path;
      return `<p class="notes-summary">${count} note${count === 1 ? "" : "s"} on <code class="reference" data-reference="${escapeHtml(where)}">${escapeHtml(target.path)}</code></p>`;
    }
    return mount(block);
  };
