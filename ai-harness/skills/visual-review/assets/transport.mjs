 ...
 */
import { describeEntry, searchPaths, treeFromIndex } from "./repo-query.mjs";
...
function answer(url) {
  ...
  const index = offline.cache[`index:${sha}`] ?? [];
  ...
  if (at.pathname === "/repo/tree") {
    const path = query.get("path") ?? "";
    // A directory nobody expanded can still be listed, because the path index
    // describes the whole tree at that commit.
    const entries =
      offline.cache[`tree:${sha}:${path}`] ?? treeFromIndex(index, path);
  ...
  if (at.pathname === "/repo/paths")
    return json({
      ...
      ...searchPaths(index, query.get("q"), query.get("limit")),
    });
