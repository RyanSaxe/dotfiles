...
- **Test Behavior, Not Wording**. Tests must exercise externally meaningful behavior or stable invariants. Never assert that documentation contains prescribed prose or magic strings; documentation completeness is a review concern, while links and executable examples may be validated directly.
...
- Use zsh, not bash. Scripts must be portable across macOS and Linux (Ubuntu/Debian). Target zsh 5.9+. Format POSIX-compatible fragments with `shfmt`.

## Reusable Tools

- One-off or throwaway: write to `/tmp/`, never the working directory.
- Agent-facing reusable tools: place in the global AI harness's `tools/` directory and expose them through `~/.local/bin`.
- Shell: zsh 5.9+, platform-agnostic.
- Python: PEP 723 inline metadata so `uv run` self-manages the environment. Use this shebang:
